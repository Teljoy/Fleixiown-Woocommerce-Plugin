...
if (isset($options['flexiown_store_locator'])) {
	$flexiown_store_locator = $options['flexiown_store_locator'];
}

if (isset($flexiown_store_locator) && $flexiown_store_locator == 'yes') {
        ...
        function enqueue_store_location_scripts() {
            ...
            wp_enqueue_script('store-location-popup', FLEXIOWN_PLUGIN_URL . 'assets/js/store-location-popup.js', array('jquery'), '1.0.0', true);
            wp_enqueue_style('store-location-popup', FLEXIOWN_PLUGIN_URL . 'assets/css/store-location-popup.css');
        }
...
function inject_store_location_popup() {
    ...
    <?php
        ...
        // Also add support for WooCommerce Blocks checkout
        add_action('wp_head', 'inject_store_location_popup_for_blocks');

}

// Function specifically for blocks checkout
function inject_store_location_popup_for_blocks() {
    ...
    // Don't render if we have an active store
    if (!empty($active_store_location_id)) {
        return;
    }
    
    // Get store locations for the popup
    $store_locations = get_posts(array(
        'post_type' => 'store_location',
        'posts_per_page' => -1,
    ));
    ...
    ?>
    <div id="store-location-popup-blocks" class="modal">
        <div class="modal-content">
            <span id="close-store-location-popup-blocks">&times;</span>
            <a href="<?php echo home_url(); ?>" id="return-to-shopping-blocks" style="position:relative; right:0px"><?php _e('Return to Shopping', 'text_domain'); ?></a>
            <hr>
            <h2><?php _e('Please Select Store in order to continue to checkout', 'text_domain'); ?></h2>
            <select id="store-location-dropdown-blocks" class="store-location-dropdown">
                <option value=""><?php _e('Select Store Location', 'text_domain'); ?></option>
                <?php foreach ($store_locations as $store_location) : ?>
                    <option value="<?php echo esc_attr($store_location->ID); ?>">
                        <?php echo esc_html($store_location->post_title); ?>
                    </option>
                <?php endforeach; ?>
            </select>
            <br><br>
            <button id="continue-to-checkout-blocks"><?php _e('Continue to Checkout', 'text_domain'); ?></button>
        </div>
    </div>
    ...
    <script>
        jQuery(document).ready(function($) {
            ...
            function initStoreLocationPopup() {
                var modal = $('#store-location-popup-blocks');
                var span = $('#close-store-location-popup-blocks');
                var continueButton = $('#continue-to-checkout-blocks');
                var storeDropdown = $('#store-location-dropdown-blocks');
                
                // Check if checkout block exists and legacy popup doesn't exist
                if ($('.wp-block-woocommerce-checkout').length > 0 && $('#store-location-popup').length === 0) {
                    // Show modal for blocks checkout
                    modal.show();
                    ...
                    // Not a blocks checkout or legacy popup exists, remove this popup
                    modal.remove();
                }
            }
            
            // Try immediately and also wait for DOM changes
            initStoreLocationPopup();
            
            // Also try after a short delay in case blocks are still loading
            setTimeout(initStoreLocationPopup, 1000);
            
            // And try again after a longer delay for slower loading
            setTimeout(initStoreLocationPopup, 3000);
        });
    </script>
    <?php
}